...
class TopNav extends Component {
  ...
  filterNames = () => {
    if (this.state.query == "") {
      ...
      }).slice(0, 5).map((collection) => (
        <ListGroup.Item action href={`/collections/${collection.slug}`} className="bg-dark text-white border-secondary">{collection.name}</ListGroup.Item>
      ))
    }
  }
